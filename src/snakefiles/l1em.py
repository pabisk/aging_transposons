...
rule run_l1em:
    ...
    input:
        ...
        genome_fasta=features["reference"]["genome_fasta"],
    output:
        "results/retrotransposons_l1em/n_{group}__{sample_prep}/L1EM-master/full_counts.txt"
    resources:
    ...
    conda:
        "l1em.yaml"
    log:
        "results/logs/retrotransposons_l1em/n_{group}__{sample_prep}/l1em.txt"
    shell:
        "cp -r data/L1EM-master results/retrotransposons_l1em/n_{wildcards.group}__{wildcards.sample_prep} ; \
        cd results/retrotransposons_l1em/n_{wildcards.group}__{wildcards.sample_prep}/L1EM-master ; \
        pwd ; \
        rm -d -r -f idL1reads ; \
        rm -d -r -f split_fqs ; \
        rm -d -r -f G_of_R ; \
        ...
        samtools --version ; \
        echo now run l1em: ; \
        bash -e run_L1EM{params.species}{params.stranded}.sh {params.pwd}/{input.bam} {params.pwd}/results/retrotransposons_l1em/n_{wildcards.group}__{wildcards.sample_prep}/L1EM-master {params.pwd}/{input.genome_fasta} {params.n}  ; \
        rm -d -r -f idL1reads ; \
        rm -d -r -f split_fqs ; \
        rm -d -r -f G_of_R"

rule l1em_rbind_filter :
        """
        this 
        """
        input: 
            in1=expand("results/retrotransposons_l1em/n_{group}__{sample_prep}/L1EM-master/full_counts.txt",zip, group=GROUPS, sample_prep = SAMPLES_prep),
        output: 
            "results/retrotransposons_l1em/l1em_final_result.csv",
            "results/retrotransposons_l1em/l1em_clean_final_result.csv",
        params:
